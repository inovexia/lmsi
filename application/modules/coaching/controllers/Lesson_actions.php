...
class Lesson_actions extends MX_Controller {
	...
	public function add_content ($coaching_id=0, $course_id=0, $lesson_id=0, $page_id=0) {
		$this->form_validation->set_rules ('title', 'Page Title', 'required|htmlspecialchars|trim');
		
		if ( $this->form_validation->run () == true )  {
			$page_id = $this->lessons_model->add_page ($coaching_id, $course_id, $lesson_id, $page_id);
			$response = $this->lessons_model->add_attachment ($coaching_id, $course_id, $lesson_id, $page_id);
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>true, 'message'=>'Content added successfully', 'redirect'=>site_url('coaching/lessons/add_page/'.$coaching_id.'/'.$course_id.'/'.$lesson_id.'/'.$page_id) )));
		} else {
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>false, 'error'=>validation_errors() )));
		} 
	...
	public function add_attachment ($coaching_id=0, $course_id=0, $lesson_id=0, $page_id=0) {
 	...
 	public function delete_attachment ($coaching_id=0, $course_id=0, $lesson_id=0, $page_id=0, $att_id=0, $att_type=0, $redirect=1) {
		...
		// Delete entry
 		...
 		if ($redirect == 1) {
 			$this->message->set('Content removed successfully', 'info', true);
