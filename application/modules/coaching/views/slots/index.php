  ...
  <div class="col-12 ">
    <form class="form-inline">
      <?php
            ...
            ?>
      <input type="date" name="" class="form-control my-2" min="<?php //echo date ('Y-m-d'); ?>"
        max="<?php echo date ('Y-m-d', strtotime('+90 days')); ?>" placeholder="Select date"
        value="<?php echo date ('Y-m-d', strtotime($date)); ?>" onchange="change_date(this.value)">
    </form>
  </div>
</div>
    ...
    <div class="card mb-3">
      <div class="card-body">
        ...
        <?php
                        if (! empty ($slots)) {
                            foreach ($slots as $slot) {
                                ?>
        <button type="button" class="btn btn-xs btn-info mb-2" data-toggle="modal" data-backdrop="static"
          data-target="#addSlot" data-course-id="<?php echo $course_id; ?>"
        ...
        <?php
                        ...
                        ?>
        <div class="my-2 mt-2">
          <button type="button" class="btn btn-xs btn-outline-primary" data-toggle="modal" data-backdrop="static"
            data-target="#addSlot" data-course-id="<?php echo $course_id; ?>" data-slot-id="0">Add Slot</button>
        </div>
      </div>
    </div>
    <?php
...
<div class="modal fade modal-right" id="addSlot" tabindex="-1" role="dialog" aria-labelledby="addSlot"
  aria-hidden="true">
