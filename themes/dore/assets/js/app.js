...
$(document).ready(function () {
  ...
  function matchPassword(conf_password) {
    if (
      ...
      $("#re_pass").removeClass();
      $("#re_pass").addClass("simple-icon-check text-success");
    } else {
      $("#re_pass").removeClass();
      $("#re_pass").addClass("simple-icon-close text-danger");
    }
  }
  function checkStrength(password) {
    ...
    if (password.match(/([a-z].*[A-Z])|([A-Z].*[a-z])/)) {
      strength += 1;
      $("#letter").removeClass();
      $("#letter").addClass("simple-icon-check text-success");
    } else {
      $("#letter").removeClass();
      $("#letter").addClass("simple-icon-close text-danger");
    }
    //If it has numbers and characters, increase strength value.
    if (password.match(/([a-zA-Z])/) && password.match(/([0-9])/)) {
      strength += 1;
      $("#number").removeClass();
      $("#number").addClass("simple-icon-check text-success");
    } else {
      $("#number").removeClass();
      $("#number").addClass("simple-icon-close text-danger");
    }
    //If it has one special character, increase strength value.
    if (password.match(/([!,%,&,@,#,$,^,*,?,_,~])/)) {
      strength += 1;
      $("#spcl_char").removeClass();
      $("#spcl_char").addClass("simple-icon-check text-success");
    } else {
      $("#spcl_char").removeClass();
      $("#spcl_char").addClass("simple-icon-close text-danger");
    }
    if (password.length > 7) {
      strength += 1;
      $("#length").removeClass();
      $("#length").addClass("simple-icon-check text-success");
    } else {
      $("#length").removeClass();
      $("#length").addClass("simple-icon-close text-danger");
    }
